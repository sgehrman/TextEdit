...
+ (BOOL)isRichTextType:(NSString *)typeName {
  /* We map all plain text documents to public.text.  Therefore a document is
   * rich iff its type is not public.text. */
  return ![typeName isEqualToString:UTTypeText.identifier];
}

+ (NSString *)readableTypeForType:(NSString *)type {
  // There is a partial order on readableTypes given by UTTypeConformsTo. We
  // linearly extend the partial order to a total order using <. Therefore we
  // can compute the ancestor with greatest level (furthest from root) by linear
  // search in the resulting array. Why do we have to do this?  Because type
  // might conform to multiple readable types, such as "public.rtf" and
  // "public.text" and "public.data" and we want to find the most specialized
  // such type.
  ...
  dispatch_once(&pred, ^{
    ...
    topologicallySortedReadableTypes = [topologicallySortedReadableTypes
        sortedArrayUsingComparator:^NSComparisonResult(id type1, id type2) {
          ...
          if ([[UTType typeWithIdentifier:type1]
                  conformsToType:[UTType typeWithIdentifier:type2]]) {
            return NSOrderedAscending;
          }
          if ([[UTType typeWithIdentifier:type2]
                  conformsToType:[UTType typeWithIdentifier:type1]]) {
  ...
  for (NSString *readableType in topologicallySortedReadableTypes) {
    if ([[UTType typeWithIdentifier:type]
            conformsToType:[UTType typeWithIdentifier:readableType]]) {
      return readableType;
    }
  }
  return nil;
...
- (id)init {
  if ((self = [super init])) {
    ...
    [self setUsesScreenFonts:[self isRichText]
                                 ? [[NSUserDefaults standardUserDefaults] boolForKey:UseScreenFonts]
                                 : YES];
...
/* Return an NSDictionary which maps Cocoa text system document identifiers (as
 * declared in AppKit/NSAttributedString.h) to document types declared in
 * TextEdit's Info.plist.
 */
- (NSDictionary *)textDocumentTypeToTextEditDocumentTypeMappingTable {
  static NSDictionary *documentMappings = nil;
  // Use of dispatch_once() makes the initialization thread-safe, and it needs
  // to be, since multiple documents can be opened concurrently
  static dispatch_once_t once = 0;
  dispatch_once(&once, ^{
    documentMappings = [[NSDictionary alloc]
        initWithObjectsAndKeys:UTTypeText.identifier, NSPlainTextDocumentType, UTTypeRTF.identifier,
                               NSRTFTextDocumentType, UTTypeRTFD.identifier, NSRTFDTextDocumentType,
                               SimpleTextType, NSMacSimpleTextDocumentType, UTTypeHTML.identifier,
                               NSHTMLTextDocumentType, Word97Type, NSDocFormatTextDocumentType,
                               Word2007Type, NSOfficeOpenXMLTextDocumentType, Word2003XMLType,
                               NSWordMLTextDocumentType, OpenDocumentTextType,
                               NSOpenDocumentTextDocumentType, UTTypeWebArchive.identifier,
                               NSWebArchiveTextDocumentType, nil];
  });
  return documentMappings;
}

/* This method is called by the document controller. The message is passed on
 * after information about the selected encoding (from our controller subclass)
 * and preference regarding HTML and RTF formatting has been added.
 * -lastSelectedEncodingForURL: returns the encoding specified in the Open
 * panel, or the default encoding if the document was opened without an open
 * panel.
 */
- (BOOL)readFromURL:(NSURL *)absoluteURL ofType:(NSString *)typeName error:(NSError **)outError {
  DocumentController *docController = [DocumentController sharedDocumentController];
  return [self readFromURL:absoluteURL
                    ofType:typeName
                  encoding:[docController lastSelectedEncodingForURL:absoluteURL]
                 ignoreRTF:[docController lastSelectedIgnoreRichForURL:absoluteURL]
                ignoreHTML:[docController lastSelectedIgnoreHTMLForURL:absoluteURL]
                     error:outError];
}
...
- (BOOL)readFromURL:(NSURL *)absoluteURL
  ...
  /* generalize the passed-in type to a type we support.  for instance,
   * generalize "public.xml" to "public.txt" */
  ...
  if (encoding != NoStringEncoding) {
    [options setObject:[NSNumber numberWithUnsignedInteger:encoding]
                forKey:NSCharacterEncodingDocumentOption];
  }
  [self setEncoding:encoding];

  // Check type to see if we should load the document as plain. Note that this
  // check isn't always conclusive, which is why we do another check below,
  // after the document has been loaded (and correctly categorized).
  if ((ignoreRTF && ([[UTType typeWithIdentifier:typeName] conformsToType:UTTypeRTF] ||
                     [[UTType typeWithIdentifier:typeName]
                         conformsToType:[UTType typeWithIdentifier:Word2003XMLType]])) ||
      (ignoreHTML && [[UTType typeWithIdentifier:typeName] conformsToType:UTTypeHTML]) ||
      [self isOpenedIgnoringRichText]) {
    [options setObject:NSPlainTextDocumentType forKey:NSDocumentTypeDocumentOption]; // Force plain
    typeName = UTTypeText.identifier;
    [self setOpenedIgnoringRichText:YES];
  }

  [[text mutableString] setString:@""];
  // Remove the layout managers while loading the text; mutableCopy retains the
  // array so the layout managers aren't released
  NSArray *layoutMgrs = [[text layoutManagers] copy];
  for (NSLayoutManager *layoutMgr in layoutMgrs) {
    [text removeLayoutManager:layoutMgr];
  }

  // We can do this loop twice, if the document is loaded as rich text although
  // the user requested plain
  BOOL retry;
  do {
    BOOL success;
    NSString *docType;
    ...
    [text beginEditing];
    success = [text readFromURL:absoluteURL
                        options:options
    ...
    // First check to see if the document was rich and should have been loaded
    // as plain
    if (![[options objectForKey:NSDocumentTypeDocumentOption]
        ...
        ((ignoreHTML && [docType isEqual:NSHTMLTextDocumentType]) ||
         (ignoreRTF && ([docType isEqual:NSRTFTextDocumentType] ||
      ...
      NSString *newFileType =
          [[self textDocumentTypeToTextEditDocumentTypeMappingTable] objectForKey:docType];
      if (newFileType) {
        typeName = newFileType;
      } else {
        typeName = UTTypeRTF.identifier; // Hmm, a new type in the Cocoa text
                                         // system. Treat it as rich. ??? Should
                                         // set the converted flag too?
      }
      if (![[self class] isRichTextType:typeName]) {
  ...
  /* If the document has a stored value for view mode, use it. Otherwise wrap to
   * window. */
  ...
  /* Pre MacOSX versions of TextEdit wrote out the view (window) size in
PaperSize. If we encounter a non-MacOSX RTF file, and it's written by TextEdit,
use PaperSize as ViewSize */
  ...
  if (viewSizeVal) {
    ...
    if (paperSizeVal) { // See if PaperSize should be used as ViewSize; if so,
      ...
      if (val && ([val integerValue] < 100)) { // Indicates old RTF file; value described in
  ...
  [self setHyphenationFactor:(val = [docAttrs objectForKey:NSHyphenationFactorDocumentAttribute])
                                 ? [val floatValue]
                                 : 0];
  [self setBackgroundColor:(val = [docAttrs objectForKey:NSBackgroundColorDocumentAttribute])
                               ? val
                               : [NSColor whiteColor]];
  ...
  for (NSString *property in [self knownDocumentProperties]) {
    [self setValue:[docAttrs objectForKey:[map objectForKey:property]]
            forKey:property]; // OK to set nil to clear
  }
  ...
  if (forRichText) {
    ...
    if (defaultRichParaStyle == nil) { // We do this once...
      ...
      NSString *measurementUnits =
          [[NSUserDefaults standardUserDefaults] objectForKey:@"AppleMeasurementUnits"];
      CGFloat tabInterval = ([@"Centimeters" isEqual:measurementUnits])
                                ? (72.0 / 2.54)
                                : (72.0 / 2.0); // Every cm or half inch
      NSMutableParagraphStyle *paraStyle = [[NSMutableParagraphStyle alloc] init];
      NSTextAlignment alignment = ((NSWritingDirectionRightToLeft ==
      ...
      for (cnt = 0; cnt < 12; cnt++) {         // Add 12 tab stops, at desired intervals...
        NSTextTab *tabStop = [[NSTextTab alloc] initWithTextAlignment:alignment
    ...
    charWidth = [@" " sizeWithAttributes:[NSDictionary dictionaryWithObject:charWidthFont
...
- (void)applyDefaultTextAttributes:(BOOL)forRichText {
  ...
  NSTextStorage *text = [self textStorage];
  // We now preserve base writing direction even for plain text, using
  // the 10.6-introduced attribute enumeration API
  [text enumerateAttribute:NSParagraphStyleAttributeName
                   inRange:NSMakeRange(0, [text length])
                   options:0
                usingBlock:^(id paragraphStyle, NSRange paragraphStyleRange, BOOL *stop) {
                  NSWritingDirection writingDirection =
                      paragraphStyle ? [(NSParagraphStyle *)paragraphStyle baseWritingDirection]
                                     : NSWritingDirectionNatural;
                  // We also preserve NSWritingDirectionAttributeName (new
                  // in 10.6)
                  [text enumerateAttribute:NSWritingDirectionAttributeName
                                   inRange:paragraphStyleRange
                                   options:0
                                usingBlock:^(id value, NSRange attributeRange, BOOL *stop) {
                                  [text setAttributes:textAttributes range:attributeRange];
                                  if (value) {
                                    [text addAttribute:NSWritingDirectionAttributeName
                                                 value:value
...
/* This method will return a suggested encoding for the document. Since Mac OS X
 * Leopard, unless the user has specified a favorite encoding for saving that
 * applies to the document, we use UTF-8.
 */
- (NSStringEncoding)suggestedDocumentEncoding {
  ...
  if (val) {
    ...
    if ((chosenEncoding != NoStringEncoding) && (chosenEncoding != NSUnicodeStringEncoding) &&
  ...
  if (enc == NoStringEncoding) {
    enc = NSUTF8StringEncoding; // Default to UTF-8
  }
  return enc;
}

/* Clear the delegates of the text views and window, then release all resources
 * and go away...
 */
...
- (void)setFileType:(NSString *)type {
  /* Due to sandboxing, we cannot (usefully) directly change the document's file
   * URL except for changing it to nil.  This means that when we convert a
   * document from rich text to plain text, our only way of updating the file
   * URL is to have NSDocument do it for us in response to a change in our file
   * type.  However, it is not as simple as setting our type to kUTTypeText, as
   * would be accurate, because if we have a rtf document, public.rtf inherits
   * from public.text and so NSDocument wouldn't change our extension (which is
   * correct, BTW, since it's also perfectly valid to open a rtf and not
   * interpret the rtf commands, in which case a save of a rtf document as
   * kUTTypeText should not change the extension).  Therefore we need to save
   * using a subtype of kUTTypeText that isn't in the path from kUTTypeText to
   * kUTTypeRTF.  The obvious candidate is kUTTypePlainText.  Therefore, we need
   * to save using kUTTypePlainText when we convert a rtf to plain text and then
   * map the file type from kTTypePlainText to kUTTypeText.  The inverse of the
   * mapping occurs here. */
  if ([type isEqualToString:UTTypePlainText.identifier]) {
...
/* Table mapping document property keys "company", etc, to text system document
 * attribute keys (NSCompanyDocumentAttribute, etc)
 */
- (NSDictionary *)documentPropertyToAttributeNameMappings {
  static NSDictionary *dict = nil;
  if (!dict) {
    dict = [[NSDictionary alloc]
        initWithObjectsAndKeys:NSCompanyDocumentAttribute, @"company", NSAuthorDocumentAttribute,
                               @"author", NSKeywordsDocumentAttribute, @"keywords",
                               NSCopyrightDocumentAttribute, @"copyright", NSTitleDocumentAttribute,
                               @"title", NSSubjectDocumentAttribute, @"subject",
...
/* If there are document properties and they are not the same as the defaults
 * established in preferences, return YES
 */
- (BOOL)hasDocumentProperties {
  for (NSString *key in [self knownDocumentProperties]) {
    id value = [self valueForKey:key];
    if (value && ![value isEqual:[[NSUserDefaults standardUserDefaults] objectForKey:key]]) {
      return YES;
    }
  }
  return NO;
...
/* This actually clears all properties (rather than setting them to default
 * values established in preferences)
 */
...
/* We implement a setValue:forDocumentProperty: to work around NSUndoManager bug
 * where prepareWithInvocationTarget: fails to freeze-dry invocations with
 * "known" methods such as setValue:forKey:.
 */
- (void)setValue:(id)value forDocumentProperty:(NSString *)property {
  ...
  [[self undoManager]
      setActionName:NSLocalizedString(property,
                                      "")]; // Potential strings for action names are listed
                                            // below (for genstrings to pick up)
...
- (void)setValue:(id)value forKey:(NSString *)key {
  if ([[self knownDocumentProperties] containsObject:key]) {
    [self setValue:value forDocumentProperty:key]; // We take a side-trip to this method to
                                                   // register for undo
  } else {
    [super setValue:value forKey:key]; // In case some other KVC call is sent to Document, we
                                       // treat it normally
  }
}

/* For genstrings:
    NSLocalizedStringWithDefaultValue(@"author", @"", @"", @"Change Author",
   @"Undo menu change string, without the 'Undo'");
    NSLocalizedStringWithDefaultValue(@"copyright", @"", @"", @"Change
   Copyright", @"Undo menu change string, without the 'Undo'");
    NSLocalizedStringWithDefaultValue(@"subject", @"", @"", @"Change Subject",
   @"Undo menu change string, without the 'Undo'");
    NSLocalizedStringWithDefaultValue(@"title", @"", @"", @"Change Title",
   @"Undo menu change string, without the 'Undo'");
    NSLocalizedStringWithDefaultValue(@"company", @"", @"", @"Change Company",
   @"Undo menu change string, without the 'Undo'");
    NSLocalizedStringWithDefaultValue(@"comment", @"", @"", @"Change Comment",
   @"Undo menu change string, without the 'Undo'");
    NSLocalizedStringWithDefaultValue(@"keywords", @"", @"", @"Change Keywords",
   @"Undo menu change string, without the 'Undo'");
*/
...
- (NSPrintOperation *)printOperationWithSettings:(NSDictionary *)printSettings
  ...
  id printingView;
  if (multiPage) { // If already in multiple-page ("wrap-to-page") mode, we
                   // simply use the display view for printing
    printingView = documentView;
    [[[[self windowControllers] objectAtIndex:0] firstTextView]
        textEditDoForegroundLayoutToCharacterIndex:NSIntegerMax]; // Make sure the whole document is
                                                                  // laid out before printing
  } else { // Otherwise we create a new text view (along with a text container
           // and layout manager)
    printingView = [[PrintingTextView alloc] init];
    NSLayoutManager *layoutManager = [[printingView textContainer] layoutManager];
    NSTextStorage *unnecessaryTextStorage =
        [layoutManager textStorage]; // We don't want the text storage, since we
                                     // will use the one we have
    [unnecessaryTextStorage removeLayoutManager:layoutManager];
    [_textStorage addLayoutManager:layoutManager];
    [printingView setLayoutOrientation:[[[[self windowControllers] objectAtIndex:0] firstTextView]
                                           layoutOrientation]];
  }
  ...
  // Since this printing view may not be embedded in a window, we have to
  // manually set the print job title.
  [op setJobTitle:[documentView printJobTitle]];

  // If we're in wrap-to-page mode, no need to let the user tweak wrap-to-page
  // mode printing
  ...
  if (!multiPage) {
    [printingView
        setOriginalSize:[[[[self windowControllers] objectAtIndex:0] firstTextView] frame].size];
    [printingView setPrintPanelAccessoryController:accessoryController];
    // We allow changing print parameters if not in "Wrap to Page" mode, where
    // the page setup settings are used
...
- (IBAction)toggleReadOnly:(id)sender {
  ...
  [[self undoManager]
      setActionName:[self isReadOnly]
                        ? NSLocalizedString(@"Allow Editing",
                                            @"Menu item to make the current document "
                                            @"editable (not read-only)")
                        : NSLocalizedString(@"Prevent Editing",
                                            @"Menu item to make the current document read-only")];
...
- (BOOL)toggleRichWillLoseInformation {
  ...
  return (([self isRichText] // Only rich -> plain can lose information.
           && ((length > 0)  // If the document contains characters and...
               && (attrs = [_textStorage attributesAtIndex:0
                                            effectiveRange:&range]) // ...they have attributes...
               && ((range.length < length) // ...which either are not the same for
                                           // the whole document...
                   || ![[self defaultTextAttributes:YES]
                          isEqual:attrs])   // ...or differ from the default, then...
               ))                           // ...we will lose styling information.
          || [self hasDocumentProperties]); // We will also lose information if
                                            // the document has properties.
}
...
- (void)toggleHyphenation:(id)sender {
  ...
  [self
      setHyphenationFactor:(currentHyphenation > 0.0) ? 0.0 : 0.9]; /* Toggle between 0.0 and 0.9 */
}
...
- (void)appendPlainTextExtensionChanged:(id)sender {
  ...
  [panel setAllowedContentTypes:[sender state] ? @[ UTTypePlainText ] : @[]];
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)aCell {
  ...
  if (action == @selector(toggleReadOnly:)) {
    ...
    validateToggleItem(
        aCell, [self hasMultiplePages],
        NSLocalizedString(@"&Wrap to Window",
    ...
    validateToggleItem(
        aCell, ([self hyphenationFactor] > 0.0),
        NSLocalizedString(@"Do not Allow Hyphenation",
                          @"Menu item to disallow hyphenation in the document"),
        NSLocalizedString(@"Allow Hyphenation", @"Menu item to allow hyphenation in the document"));
    if ([self isReadOnly]) {
      return NO;
    }
    return YES;
...
- (BOOL)revertToContentsOfURL:(NSURL *)url ofType:(NSString *)type error:(NSError **)outError {
  ...
  if (success) {
    if (_fileTypeToSet) { // If we're reverting, NSDocument will set the file
                          // type behind out backs. This enables restoring that
                          // type.
...
/* Target/action method for saving as (actually "saving to") PDF. Note that this
 * approach of omitting the path will not work on Leopard; see TextEdit's
 * README.rtf
 */
- (IBAction)saveDocumentAsPDFTo:(id)sender {
  [self printDocumentWithSettings:[NSDictionary dictionaryWithObjectsAndKeys:NSPrintSaveJob,
...
+ (BOOL)canConcurrentlyReadDocumentsOfType:(NSString *)typeName {
  return !([[UTType typeWithIdentifier:typeName] conformsToType:UTTypeHTML] ||
           [[UTType typeWithIdentifier:typeName] conformsToType:UTTypeWebArchive]);
}

- (void)makeWindowControllers {
  NSArray *myControllers = [self windowControllers];

  // If this document displaced a transient document, it will already have been
  // assigned a window controller. If that is not the case, create one.
  if ([myControllers count] == 0) {
    [self addWindowController:[[DocumentWindowController alloc] init]];
  }
}

/* One of the determinants of whether a file is locked is whether its type is
 * one of our writable types. However, the writable types are a function of
 * whether the document contains attachments. But whether we are locked cannot
 * be a function of whether the document contains attachments, because we won't
 * be asked to redetermine autosaving safety after an undo operation resulting
 * from a cancel, so the document would continue to appear locked if an image
 * were dragged in and then cancel was pressed.  Therefore, we must use an
 * "ignoreTemporary" boolean to treat RTF as temporarily writable despite the
 * attachments.  That's fine since
 * -checkAutosavingSafetyAfterChangeAndReturnError: will perform this check
 * again with ignoreTemporary set to NO, and that method will be called when the
 * operation is done and undone, so no inconsistency results.
 */
- (NSArray *)writableTypesForSaveOperation:(NSSaveOperationType)saveOperation
  ...
  if (saveOperation == NSSaveAsOperation) {
    ...
    if ([self isRichText]) {
      [outArray removeObject:UTTypeText.identifier];
      [outArray removeObject:UTTypePlainText.identifier];
    }

    // Documents that contain attachments can only be saved in formats that
    // support embedded graphics.
    if (!ignoreTemporary && [_textStorage containsAttachments]) {
      [outArray setArray:[NSArray arrayWithObjects:UTTypeRTFD.identifier,
                                                   UTTypeWebArchive.identifier, nil]];
    }
  }
  return outArray;
}
...
- (NSString *)fileNameExtensionForType:(NSString *)inTypeName
                         saveOperation:(NSSaveOperationType)inSaveOperation {
  /* We use kUTTypeText as our plain text type.  However, kUTTypeText is really
   * a class of types and therefore contains no preferred extension.  Therefore
   * we must specify a preferred extension, that of kUTTypePlainText. */
  if ([inTypeName isEqualToString:UTTypeText.identifier]) {
    return @"txt";
  }
  return [super fileNameExtensionForType:inTypeName saveOperation:inSaveOperation];
}

/* When we save, we send a notification so that views that are currently
coalescing undo actions can break that. This is done for two reasons, one
technical and the other HI oriented.

Firstly, since the dirty state tracking is based on undo, for a coalesced set of
changes that span over a save operation, the changes that occur between the save
and the next time the undo coalescing stops will not mark the document as dirty.
Secondly, allowing the user to undo back to the precise point of a save is good
UI.

In addition we overwrite this method as a way to tell that the document has been
saved successfully. If so, we set the save time parameters in the document.
*/
- (void)saveToURL:(NSURL *)absoluteURL
  ...
  // means the user's undo of long typing will take them back to the last spot
  // an autosave occured. This might seem confusing, and a more elaborate
  // solution may be possible (cause an autosave without having to
  // breakUndoCoalescing), but since this change is coming late in Leopard, we
  // decided to go with the lower risk fix.
  ...
  [super saveToURL:absoluteURL
      ...
      completionHandler:^(NSError *error) {
        [self setEncodingForSaving:NoStringEncoding]; // This is set during prepareSavePanel:,
                                                      // but should be cleared for future save
                                                      // operation without save panel
...
- (NSError *)errorInTextEditDomainWithCode:(NSInteger)errorCode {
  ...
  case TextEditSaveErrorWritableTypeRequired: {
    ...
    /* the document can't be saved in its original format, either because
       TextEdit cannot write to the format, or TextEdit cannot write documents
       containing attachments to the format. */
    if ([_textStorage containsAttachments]) {
      description =
          NSLocalizedString(@"Convert this document to RTFD format?",
                            @"Title of alert panel prompting the user to convert to RTFD.");
      recoverySuggestion =
          NSLocalizedString(@"Documents with graphics and attachments will be saved using RTFD "
                            @"(RTF with graphics) format. RTFD documents are not compatible with "
                            @"some applications. Convert anyway?",
                            @"Contents of alert panel prompting the user to convert to RTFD.");
    } else {
      description =
          NSLocalizedString(@"Convert this document to RTF format?",
                            @"Title of alert panel prompting the user to convert to RTF.");
      recoverySuggestion =
          NSLocalizedString(@"This document must be converted to RTF before it can be modified.",
                            @"Contents of alert panel prompting the user to convert to RTF.");
    }
    return [NSError
        errorWithDomain:TextEditErrorDomain
                   code:TextEditSaveErrorWritableTypeRequired
               userInfo:[NSDictionary
                            dictionaryWithObjectsAndKeys:
                                description, NSLocalizedDescriptionKey, recoverySuggestion,
                                NSLocalizedRecoverySuggestionErrorKey,
                                [NSArray arrayWithObjects:NSLocalizedString(
                                                              @"Convert",
                                                              @"Button choice that allows the "
                                                              @"user to convert the document."),
                                                          NSLocalizedString(
                                                              @"Cancel",
                                                              @"Button choice that allows the "
                                                              @"user to cancel."),
                                                          NSLocalizedString(
                                                              @"Duplicate",
                                                              @"Button choice that allows the "
                                                              @"user to duplicate the document."),
                                                          nil],
                                NSLocalizedRecoveryOptionsErrorKey, self,
                                NSRecoveryAttempterErrorKey, nil]];
  }
  case TextEditSaveErrorConvertedDocument: {
    NSString *newFormatName =
        [_textStorage containsAttachments]
            ? NSLocalizedString(@"rich text with graphics (RTFD)",
                                @"Rich text with graphics file format name, "
                                @"displayed in alert")
            : NSLocalizedString(@"rich text", @"Rich text file format name, displayed in alert");
    return [NSError
        errorWithDomain:TextEditErrorDomain
                   code:TextEditSaveErrorConvertedDocument
               userInfo:[NSDictionary
                            dictionaryWithObjectsAndKeys:
                                NSLocalizedString(@"Are you sure you want to edit this document?",
                                                  @"Title of alert panel asking the user whether "
                                                  @"he wants to edit a converted document."),
                                NSLocalizedDescriptionKey,
                                [NSString
                                    stringWithFormat:NSLocalizedString(
                                                         @"This document was converted from a "
                                                         @"format that TextEdit cannot save. It "
                                                         @"will be saved in %@ format.",
                                                         @"Contents of alert panel informing "
                                                         @"user that the document is converted "
                                                         @"and cannot be written in its original "
                                                         @"format."),
                                                     newFormatName],
                                NSLocalizedRecoverySuggestionErrorKey,
                                [NSArray
                                    arrayWithObjects:NSLocalizedString(
                                                         @"Edit",
                                                         @"Button choice that allows the user to "
                                                         @"save the document."),
  ...
  case TextEditSaveErrorLossyDocument: {
    return [NSError
        errorWithDomain:TextEditErrorDomain
                   code:TextEditSaveErrorLossyDocument
               userInfo:[NSDictionary
                            dictionaryWithObjectsAndKeys:
                                NSLocalizedString(@"Are you sure you want to modify the document "
                                                  @"in place?",
                                                  @"Title of alert panel which brings up a "
                                                  @"warning about saving over the same document"),
                                NSLocalizedDescriptionKey,
                                NSLocalizedString(@"Modifying the document in place might cause "
                                                  @"you to lose some of the original formatting.  "
                                                  @"Would you like to duplicate the document "
                                                  @"first?",
                                                  @"Contents of alert panel informing user that "
                                                  @"they need to supply a new file name because "
                                                  @"the save might be lossy"),
                                NSLocalizedRecoverySuggestionErrorKey,
                                [NSArray
                                    arrayWithObjects:NSLocalizedString(
                                                         @"Duplicate",
                                                     ...
                                                     NSLocalizedString(
                                                         @"Overwrite",
                                                         @"Button choice allowing user to "
  ...
  case TextEditSaveErrorEncodingInapplicable: {
    ...
    return [NSError
        errorWithDomain:TextEditErrorDomain
                   code:TextEditSaveErrorEncodingInapplicable
               userInfo:[NSDictionary
                            dictionaryWithObjectsAndKeys:
                                [NSString
                                    stringWithFormat:NSLocalizedString(
                                                         @"This document can no longer be saved "
                                                         @"using its original %@ encoding.",
                                                         @"Title of alert panel informing user "
                                                         @"that the file's string encoding needs "
                                                         @"to be changed."),
                                                     [NSString localizedNameOfStringEncoding:enc]],
                                NSLocalizedDescriptionKey,
                                NSLocalizedString(@"Please choose another encoding (such as "
                                                  @"UTF-8).",
                                                  @"Subtitle of alert panel informing user that "
                                                  @"the file's string encoding needs to be "
                                                  @"changed"),
                                NSLocalizedRecoverySuggestionErrorKey,
                                NSLocalizedString(@"The specified text encoding isn\\U2019t "
                                                  @"applicable.",
                                                  @"Failure reason stating that the text encoding "
                                                  @"is not applicable."),
                                NSLocalizedFailureReasonErrorKey,
                                [NSArray
                                    arrayWithObjects:NSLocalizedString(@"OK", @"OK"),
                                                     NSLocalizedString(@"Cancel",
                                                                       @"Button choice that allows "
                                                                       @"the user to cancel."),
                                                     nil],
...
- (BOOL)checkAutosavingSafetyAfterChangeAndReturnError:(NSError **)outError {
  ...
  if ([self fileURL]) {
    if (![[self writableTypesForSaveOperation:NSSaveAsOperation] containsObject:[self fileType]]) {
      ...
      if (encoding != NoStringEncoding &&
          ![[_textStorage string] canBeConvertedToEncoding:encoding]) {
        if (outError) {
...
- (void)updateChangeCount:(NSDocumentChangeType)change {
  ...
  if (change == NSChangeDone || change == NSChangeRedone) {
    ...
    // consulting the user. NSDocument will update the extension of our
    // autosaving location. If we don't do this, we won't be able to store
    // images in autosaved untitled documents.
    if (![self fileURL]) {
      if (![[self writableTypesForSaveOperation:NSSaveAsOperation]
              containsObject:[self fileType]]) {
        [self setFileType:([_textStorage containsAttachments] ? UTTypeRTFD.identifier
      ...
      if (sheetWindow) {
        [self performActivityWithSynchronousWaiting:YES
                                         usingBlock:^(void (^activityCompletionHandler)()) {
                                           void (^recoveryBlock)(BOOL) = ^(BOOL didRecover) {
                                             if (!didRecover) {
                                           ...
                                           [self presentError:error
                                               ...
                                               (didPresentErrorWithRecovery:block:)
...
/* When we duplicate a document, we need to temporarily return the autosaving
   file type for the resultant document.  Unfortunately, the only way to do this
   from a document subclass appears to be to use a boolean indicator.
 */
...
- (void)document:(NSDocument *)ignored didSave:(BOOL)didSave block:(void *)blockPtr {
  void (^block)(BOOL) = (__bridge_transfer id)blockPtr;
  block(didSave);
}

- (void)didPresentErrorWithRecovery:(BOOL)didRecover block:(void *)blockPtr {
  void (^block)(BOOL) = (__bridge_transfer id)blockPtr;
  block(didRecover);
}
...
- (void)attemptRecoveryFromError:(NSError *)error
    ...
    case TextEditSaveErrorLossyDocument:
      ...
      if (((errorCode == TextEditSaveErrorLossyDocument) && (recoveryOptionIndex == 0)) ||
        ...
        if (!duplicateDocument) {
          ...
          if (sheetWindow) {
            void (^recoveryBlock)(BOOL) = ^(BOOL didRecover) {
              ((void (*)(id, SEL, BOOL, void *))objc_msgSend)(delegate, didRecoverSelector, NO,
                                                              contextInfo);
            };
            [self presentError:duplicateError
                ...
                didPresentSelector:@selector(didPresentErrorWithRecovery:block:)
                       contextInfo:(__bridge_retained void *)recoveryBlock];
            return;
          } else {
            [self presentError:duplicateError];
          }
    ...
    case TextEditSaveErrorEncodingInapplicable:
      if (recoveryOptionIndex == 0) { // OK button
        [self continueActivityUsingBlock:^(void) {
          void (^saveBlock)(BOOL) = ^(BOOL didSave) {
            ((void (*)(id, SEL, BOOL, void *))objc_msgSend)(delegate, didRecoverSelector, didSave,
                                                            contextInfo);
          };
          [self runModalSavePanelForSaveOperation:NSSaveOperation
                                         delegate:self
                                  didSaveSelector:@selector(document:didSave:block:)
                                      contextInfo:(__bridge_retained void *)saveBlock];
        }];
        return;
      }
      break;
  ...
  ((void (*)(id, SEL, BOOL, void *))objc_msgSend)(delegate, didRecoverSelector, didRecover,
                                                  contextInfo);
}
...
- (id)fileWrapperOfType:(NSString *)typeName error:(NSError **)outError {
  ...
  NSMutableDictionary *dict = [NSMutableDictionary
      dictionaryWithObjectsAndKeys:[NSValue valueWithSize:[self paperSize]],
                                   NSPaperSizeDocumentAttribute,
                                   [NSNumber numberWithInteger:[self isReadOnly] ? 1 : 0],
                                   ...
                                   [NSNumber numberWithInteger:[self hasMultiplePages] ? 1 : 0],
  ...
  // TextEdit knows how to save all these types, including their super-types. It
  // does not know how to save any of their potential subtypes. Hence, the
  // conformance check is the reverse of the usual pattern. UTTypePlainText also
  // handles UTTypeText and has to come before the other types so we will use
  // the least specialized type
  if ([UTTypePlainText conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSPlainTextDocumentType;
  } else if ([UTTypeRTF conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSRTFTextDocumentType;
  } else if ([UTTypeRTFD conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSRTFDTextDocumentType;
  } else if ([[UTType typeWithIdentifier:SimpleTextType]
                 conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSMacSimpleTextDocumentType;
  } else if ([[UTType typeWithIdentifier:Word97Type]
                 conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSDocFormatTextDocumentType;
  } else if ([[UTType typeWithIdentifier:Word2007Type]
                 conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSOfficeOpenXMLTextDocumentType;
  } else if ([[UTType typeWithIdentifier:Word2003XMLType]
                 conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSWordMLTextDocumentType;
  } else if ([[UTType typeWithIdentifier:OpenDocumentTextType]
                 conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSOpenDocumentTextDocumentType;
  } else if ([UTTypeHTML conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSHTMLTextDocumentType;
  } else if ([UTTypeWebArchive conformsToType:[UTType typeWithIdentifier:typeName]]) {
    docType = NSWebArchiveTextDocumentType;
  } else {
    [NSException raise:NSInvalidArgumentException
                format:@"%@ is not a recognized document type.", typeName];
  }

  if (docType) {
    [dict setObject:docType forKey:NSDocumentTypeDocumentAttribute];
  }
  if ([self hasMultiplePages] && ([self scaleFactor] != 1.0)) {
    [dict setObject:[NSNumber numberWithDouble:[self scaleFactor] * 100.0]
  ...
  if (docType == NSPlainTextDocumentType) {
    enc = [self encoding];
    if ((_currentSaveOperation == NSSaveOperation || _currentSaveOperation == NSSaveAsOperation) &&
        (_encodingForSaving != NoStringEncoding)) {
      enc = _encodingForSaving;
    }
    if (enc == NoStringEncoding) {
      enc = [self suggestedDocumentEncoding];
    }
    [dict setObject:[NSNumber numberWithUnsignedInteger:enc]
    ...
    if (![defaults boolForKey:UseTransitionalDocType]) {
      [excludedElements
          addObjectsFromArray:[NSArray arrayWithObjects:@"APPLET", @"BASEFONT", @"CENTER", @"DIR",
                                                        @"FONT", @"ISINDEX", @"MENU", @"S",
                                                        @"STRIKE", @"U", nil]];
    }
  ...
  for (NSString *property in [self knownDocumentProperties]) {
    ...
    if (value && ![value isEqual:@""] && ![value isEqual:[NSArray array]]) {
      [dict setObject:value
               forKey:[[self documentPropertyToAttributeNameMappings] objectForKey:property]];
    }
  }
  ...
  if (docType == NSRTFDTextDocumentType ||
      (docType == NSPlainTextDocumentType &&
       ![self isOpenedIgnoringRichText])) { // We obtain a file wrapper from the
                                            // text storage for RTFD (to produce
                                            // a directory), or for true
                                            // plain-text documents (to write out
                                            // encoding in extended attributes)
    result = [text fileWrapperFromRange:range
                     documentAttributes:dict
                                  error:outError]; // returns NSFileWrapper
  } else {
    NSData *data = [text dataFromRange:range
                    documentAttributes:dict
    ...
    if (data) {
      ...
      if (!result && outError) {
        *outError = [NSError errorWithDomain:NSCocoaErrorDomain
                                    ...
                                    userInfo:nil]; // Unlikely, but just in case we
                                                   // should generate an NSError
      }
    }
  }
  if (result && docType == NSPlainTextDocumentType &&
      (_currentSaveOperation == NSSaveOperation || _currentSaveOperation == NSSaveAsOperation)) {
...
- (BOOL)checkAutosavingSafetyAndReturnError:(NSError **)outError {
  ...
  if ([self fileURL]) {
    // If the document is converted or lossy but can't be saved in its file
    // type, we will need to save it in a different location or duplicate it
    // anyway.  Therefore, we should tell the user that a writable type is
    // required instead.
    if (![[self writableTypesForSaveOperation:NSSaveAsOperation
      ...
      if (outError) {
        *outError = [self errorInTextEditDomainWithCode:TextEditSaveErrorConvertedDocument];
      }
      safe = NO;
    } else if ([self isLossy]) {
      if (outError) {
        *outError = [self errorInTextEditDomainWithCode:TextEditSaveErrorLossyDocument];
      }
      safe = NO;
    }
  }
...
/* For plain-text documents, we add our own accessory view for selecting
 * encodings. The plain text case does not require a format popup.
 */
 ...
 * set the default directory to the directory in which the source file
 * (converted file) resided at the time the document was converted. If the
 * document is plain text, we additionally add an encoding popup.
 */
- (BOOL)prepareSavePanel:(NSSavePanel *)savePanel {
  ...
  if (![self isRichText]) {
    BOOL addExt =
        [[NSUserDefaults standardUserDefaults] boolForKey:AddExtensionToNewPlainTextFiles];
    ...
    [self setEncodingForSaving:(enc == NoStringEncoding || ![string canBeConvertedToEncoding:enc])
                                   ? [self suggestedDocumentEncoding]
                                   : enc];
    [savePanel setAccessoryView:[[[NSDocumentController sharedDocumentController] class]
                                      encodingAccessory:[self encodingForSaving]
    ...
    // Set up the checkbox
    [extCheckbox
        setTitle:NSLocalizedString(@"If no extension is provided, use \\U201c.txt\\U201d.",
                                   @"Checkbox indicating that if the user does not specify an "
                                   @"extension when saving a plain text file, .txt will be used")];
    [extCheckbox
        setToolTip:NSLocalizedString(@"Automatically append \\U201c.txt\\U201d to the file "
                                     @"name if no known file name extension is provided.",
                                     @"Tooltip for checkbox indicating that if the user does "
                                     @"not specify an extension when saving a plain text "
                                     @"file, .txt will be used")];
    ...
    if (addExt) {
      ...
      // which gives the fileName a ".txt" extension. We want don't want to
      // append the extension for Untitled documents. First we clear out the
      // allowedFileType that NSDocument set. We want to allow anything, so we
      // pass 'nil'. This will prevent NSSavePanel from appending an extension.
      ...
      // If the document has not yet been seaved, or we couldn't find the
      // fileName, then use the displayName.
    ...
    if (cnt * [string length] < 5000000) { // Otherwise it's just too slow; would be nice to make
                                           // this more dynamic. With large docs and many encodings,
                                           // the items just won't be validated.
      while (cnt--) {                      // No reason go backwards except to use one variable
                                           // instead of two
        NSStringEncoding encoding = (NSStringEncoding)[
            [[encodingPopup itemAtIndex:cnt] representedObject] unsignedIntegerValue];
        // Hardwire some encodings known to allow any content
        if ((encoding != NoStringEncoding) && (encoding != NSUnicodeStringEncoding) &&
            (encoding != NSUTF8StringEncoding) && (encoding != NSNonLossyASCIIStringEncoding) &&
