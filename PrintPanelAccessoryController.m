...
/*
 ...
 Abstract: PrintPanelAccessoryController is a subclass of NSViewController demonstrating how to add
 an accessory view to the print panel.
...
/* The first time the printInfo is supplied, initialize the value of the pageNumbering setting from
 * defaults
 */
- (void)setRepresentedObject:(id)printInfo {
  ...
  [self setWrappingToFit:[[[NSUserDefaults standardUserDefaults] objectForKey:WrapToFitWhenPrinting]
                             boolValue]];
}
...
- (NSArray *)localizedSummaryItems {
  ...
  [items addObject:[NSDictionary
                       dictionaryWithObjectsAndKeys:
                           NSLocalizedStringFromTable(
                               @"Header and Footer", @"PrintPanelAccessory",
                               @"Print panel summary item title for whether header and footer "
                               @"(page number, date, document title) should be printed"),
                           NSPrintPanelAccessorySummaryItemNameKey,
                           [self pageNumbering]
                               ? NSLocalizedStringFromTable(
                                     @"On", @"PrintPanelAccessory",
                                     @"Print panel summary value for feature that is enabled")
                               : NSLocalizedStringFromTable(
                                     @"Off", @"PrintPanelAccessory",
                                     @"Print panel summary value for feature that is disabled"),
                           NSPrintPanelAccessorySummaryItemDescriptionKey, nil]];
  // We add the "Rewrap to fit page" item to the summary only if the item is settable (which it
  // isn't, for "wrap-to-page" mode)
  if ([self showsWrappingToFit]) {
    [items addObject:[NSDictionary
                         dictionaryWithObjectsAndKeys:
                             NSLocalizedStringFromTable(
                                 @"Rewrap to fit page", @"PrintPanelAccessory",
                                 @"Print panel summary item title for whether document contents "
                                 @"should be rewrapped to fit the page"),
                             NSPrintPanelAccessorySummaryItemNameKey,
                             [self wrappingToFit]
                                 ? NSLocalizedStringFromTable(
                                       @"On", @"PrintPanelAccessory",
