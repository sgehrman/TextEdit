...
/*
 ...
 Abstract: "Document Properties" panel controller for TextEdit.  There is a little more code here
 than one would like, however, this code does show steps needed to implement a non-modal inspector
 panel using bindings, and have the fields in the panel correctly commit when the panel loses key,
 or the document it is associated with is saved or made non-key (inactive).
 ...
 * the mainWindow, or the mainWindow's document change, we check to see what changed.  Note that
 * activeDocumentChanged doesn't mean document contents changed, but rather we have a new active
 * document.
 */
- (void)activeDocumentChanged {
  ...
  if (doc != _inspectedDocument) {
    ...
    [self setValue:(doc && [doc isKindOfClass:[Document class]]) ? doc : nil
            forKey:@"inspectedDocument"];
  }
}
 ...
 */
- (void)windowDidLoad {
  // Once the UI is loaded, we start observing the panel itself to commit editing when it becomes
  // inactive (loses key state)
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(documentPropertiesPanelDidResignKey:)
                                               name:NSWindowDidResignKeyNotification
                                             object:[self window]];

  // Make sure we start inspecting the document that is currently active, and start observing
  // changes
  [self activeDocumentChanged];
  [NSApp addObserver:self
          forKeyPath:@"mainWindow.windowController.document"
             options:0
             context:(__bridge void *)[DocumentPropertiesPanelController class]];
...
/* Since we want the panel to toggle... Note that if the window is visible and key, we order it out;
 * otherwise we make it key.
 */
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem {
  if ([menuItem action] == @selector
      (toggleWindow:)) { // Correctly toggle the menu item for showing/hiding document properties
    // We call [self isWindowLoaded] first since it prevents [self window] from loading the nib
    validateToggleItem(
        menuItem, [self isWindowLoaded] && [[self window] isVisible],
        NSLocalizedString(@"Hide Properties",
                          @"Title for menu item to hide the document properties panel."),
        NSLocalizedString(@"Show Properties",
                          @"Title for menu item to show the document properties panel (should be "
                          @"the same as the initial menu item in the nib)."));
  }
  return YES;
}
