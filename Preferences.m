...
/*
     File: Preferences.m
 Abstract: Preferences controller, subclass of NSWindowController. Since the switch to a
 bindings-based preferences interface, the class has become a lot simpler; its only duties now are
 to manage the user fonts for rich and plain text documents, translate HTML saving options from
 backwards-compatible defaults values into pop-up menu item tags, and revert everything to the
 initial defaults if the user so chooses.

 The Preferences instance also acts as a delegate for the window, in order to validate edits before
 it closes, and for the two text fields bound to the window size in characters, so that invalid
 entries trigger a reset to a field's previous value.
...
/* The user chooses the HTML document type using a popup button, but the actual type is represented
 * as a two-bit bitfield, where one bit represents whether or not to use a transitional DTD and
 * another bit determines whether or not to use XHTML. The popup button uses the bitfield's integer
 * value as its tag.
 */
- (HTMLDocumentTypeOptions)HTMLDocumentType {
  ...
  if ([defaults boolForKey:UseXHTMLDocType]) {
    type |= HTMLDocumentTypeOptionUseXHTML;
  }
  if ([defaults boolForKey:UseTransitionalDocType]) {
    type |= HTMLDocumentTypeOptionUseTransitional;
  }
...
- (void)setHTMLDocumentType:(HTMLDocumentTypeOptions)newType {
  ...
  [defaults setBool:((newType & HTMLDocumentTypeOptionUseXHTML) == HTMLDocumentTypeOptionUseXHTML)
             forKey:UseXHTMLDocType];
  [defaults setBool:((newType & HTMLDocumentTypeOptionUseTransitional) ==
                     HTMLDocumentTypeOptionUseTransitional)
             forKey:UseTransitionalDocType];
}

/* The style mode is how style information is encoded when saving HTML: using embedded or inline
 * CSS, or using older HTML tags and attributes. For backwards compatibility this information is
 * stored in user defaults as two boolean values, rather than a style mode name or enumerated
 * integer value.
 */
...
- (IBAction)revertToDefault:(id)sender {
  ...
  [[NSUserDefaultsController sharedUserDefaultsController]
      revertToInitialValues:nil]; // For the rest of the defaults
}

#pragma mark *** Window delegation ***

/* We do this to catch the case where the user enters a value into one of the text fields but closes
 * the window without hitting enter or tab.
 */
...
/* Handle the case when the user enters a ridiculous value for the window size. We just set it back
