...
- (void)openFile:(NSPasteboard *)pboard userData:(NSString *)data error:(NSString **)error {
    ...
    NSURL *url = nil;
    NSString *type = [pboard availableTypeFromArray:[NSArray arrayWithObject:UTTypePlainText.identifier]];
    ...
    if (type && (filename = origFilename = [pboard stringForType:type])) {
        ...
        if (!url) {
            // Check to see if the user mistakenly included a carriage return or more at the end of the file name...
            ...
            if ([filename hasPrefix:@"~"]) filename = [filename stringByExpandingTildeInPath];
            if (![origFilename isEqual:filename] && [filename isAbsolutePath]) {
                url = [NSURL fileURLWithPath:filename];
            }
        }
        if (url) {
            [[NSDocumentController sharedDocumentController] openDocumentWithContentsOfURL:url display:YES completionHandler:^(NSDocument *document, BOOL documentWasAlreadyOpen, NSError *openError) {
                if (!document) {
                    NSError *alertError = openError ?: [NSError errorWithDomain:NSCocoaErrorDomain code:NSFileReadInvalidFileNameError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:truncatedString(filename, PATH_MAX+10), NSFilePathErrorKey, nil]];
                    [[NSAlert alertWithError:alertError] runModal];
                }
            }];
        } else {
            NSError *alertError = [NSError errorWithDomain:NSCocoaErrorDomain code:NSFileReadInvalidFileNameError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:truncatedString(filename, PATH_MAX+10), NSFilePathErrorKey, nil]];
            [[NSAlert alertWithError:alertError] runModal];
        }
    }
}
