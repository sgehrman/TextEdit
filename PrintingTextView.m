...
/* Override of knowsPageRange: checks printing parameters against the last invocation, and if not
 * the same, resizes the view and relays out the text.  On first invocation, the saved size will be
 * 0,0, which will cause the text to be laid out.
 */
- (BOOL)knowsPageRange:(NSRangePointer)range {
  NSSize documentSizeInPage =
      documentSizeForPrintInfo([self.printPanelAccessoryController representedObject]);
  BOOL wrappingToFit = self.printPanelAccessoryController.wrappingToFit;

  if (!NSEqualSizes(_previousValueOfDocumentSizeInPage, documentSizeInPage) ||
    ...
    [[[self textContainer] layoutManager]
        setDefaultAttachmentScaling:wrappingToFit ? NSImageScaleProportionallyDown
                                                  : NSImageScaleNone];
    [self textEditDoForegroundLayoutToCharacterIndex:NSIntegerMax]; // Make sure the whole document
                                                                    // is laid out
  }
